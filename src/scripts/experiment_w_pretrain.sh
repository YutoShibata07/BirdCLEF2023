...
# 2021, 22事前学習の設定作成
python utils/make_config.py --batch_size 128 64 --lr_max 1e-3 --lr_min 1e-5  --max_epoch 30 30 --model bird_sed bird_sed --do_mixup True --training_year 2021_2022 2021_2022 

pretrain_files="../result/*batch_size=64*"
for pretrain_filepath in $pretrain_files; do
    if [ -d $pretrain_filepath ] ; then
        ...
        python utils/make_config.py --batch_size 32 64 --lr_max 1e-3 --lr_min 1e-5  --max_epoch 30 30 --model bird_sed bird_sed --do_mixup True --model_path $pretrain_filepath $pretrain_filepath
