...
def get_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--csv_dir", type=str, default="../csv_2021_2022")
...
def get_val_files(dataset_dir: str = "", input_feature: str = "logmel"):
    files_2023 = np.concatenate(
        [np.load("../csv/val_files.npy"), np.load("../csv/train_files.npy")]
    )
    ...
    for bird_path in all_species:
        ...
        bird_samples = [
            sample
            for sample in bird_samples
            if "/".join(sample.split("/")[-2:]) not in files_2023
        ]
