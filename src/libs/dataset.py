...
def get_dataloader(
    ...
    data = BirdClefDataset_maxpool(files=files, transform=transform, bird_label_map=bird_label_map, split=split, aug_list = aug_list, duration=duration, cleaning_path=cleaning_path)
...
class BirdClefDataset_maxpool(Dataset):
    def __init__(
        self,
        files: List,
        transform: Optional[transforms.Compose] = None,
    ...
    def get_sound(self, file_path: int):
        if self.split == 'train':
            sound = np.load(file_path)
            # 5s * 6 
            image_num = sound.shape[0]
            if image_num < 6:
                for i in range(6-image_num):
                    sound = np.concatenate([sound, sound[:1, :,:]], axis = 0)
            else:
                sound = sound[:6,:,:]
        elif self.split == 'oof':
            sec = int(file_path.split('_')[-1].split('.npy')[0])
            file_path = file_path.split('_')[-2] + '.npy'
            sound = np.load(file_path)
            # 5s * 6 
            image_num = sound.shape[0]
            sound = sound[sec:sec+1,:,:]
            for i in range(5):
                sound = np.concatenate([sound, sound[sec:sec+1, :,:]], axis = 0)
        else:
            sound = np.load(file_path)
            # 5s * 6 
            image_num = sound.shape[0]
    ...
    def __getitem__(self, idx: int): 
        ...
        if self.split == 'train':
            ...
            if ('soundscape' in self.aug_list) & (np.random.rand() > 0.5):
                ...
                if np.random.rand() > 0.75:
                    sound = soundscapes.copy()
                    sound = np.stack([sound, sound, sound, sound, sound, sound], axis = 0)
                    labels = np.zeros(len(self.bird_label_dict.keys()), dtype=float) + 0.001
                    labels[-1] = 1.0
                else:
                    ratio = 0.5 * np.random.rand()
            ...
            if ('white' in self.aug_list) & (np.random.rand() > 0.8):
                noise_level = 0.05
                noise = (np.random.sample((sound.shape[0], sound.shape[1], sound.shape[2])) + 9) * sound.mean() * noise_level * (np.random.sample() + 0.3)
                sound = sound + noise
            if ('pink' in self.aug_list) & (np.random.rand() > 0.8):
                r = random.randint(1,128)
                # [1, 0.98795181, ..., 0]
                pink_noise = np.array([np.concatenate((1 - np.arange(r)/r,np.zeros(128 - r)))]).T
                sound = sound + (np.random.sample((sound.shape[0], sound.shape[1], sound.shape[2])).astype(np.float32) + 9) * 2 * pink_noise * sound.mean() * 0.05 * (np.random.sample() + 0.3)
            if ('upper_freq_decay' in self.aug_list) & (np.random.rand() > 0.5):
                r = random.randint(128//2,128)
                x = random.random()/2
                pink_noise = np.array([np.concatenate((1-np.arange(r)*x/r,np.zeros(128-r)-x+1))]).T
            ...
            if ('bandpass' in self.aug_list) & (np.random.rand() > 0.3):
                start = random.randint(1, 128-10)
                leng = random.randint(0, 128-start)
                sound[:,start:start + leng,:] = sound[:,start:start + leng,:] + (np.random.sample((sound.shape[0], leng, sound.shape[2])).astype(np.float32) + 9) * 2 * sound.mean() * 0.05 * (np.random.sample() + 0.3)
        sound = mono_to_color(sound)
        sound = sound.astype(float)
        sound = np.stack([sound], axis = 1) #[6, 1, freq, frames]
        if self.transform is not None:
            sound = self.transform(sound)
        # sound = sound.transpose(2,1,0)
        # if (self.split == 'train') & ('cutout' in self.aug_list):
