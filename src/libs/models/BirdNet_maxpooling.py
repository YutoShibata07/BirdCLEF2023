...
class BirdNetwMaxpool(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.backbone(x) #(bs*part_size, 3, time_segment, freq)→(bs*part_size, in_features)
        x = x.view((-1, self.part_size, self.in_features,)) #(bs*part_size, in_features) → (bs, part_size, in_feature)
        x = x.permute(0, 2, 1)
