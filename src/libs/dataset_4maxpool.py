...
class BirdClefDataset(Dataset):
    ...
    def __getitem__(self, idx: int):        
        ...
        elif sound_size < sounds.shape[0]:
            start_idx = np.random.choice(sounds.shape[0] - sound_size)
            sounds = sounds[start_idx:start_idx+sound_size]
        #p = Pool(10)
        sounds = [sound for sound in sounds]

        #TODO Data augumentation処理
        """if self.split == 'train':
        ...
        sounds = list(map(mono_to_color, sounds))
